...
void trx_samples(set_sdr *device, int16_t *samples, int size_sample)
{
    ...
    for (size_t blk = 0; blk < total_blocks; blk++)
    {
        ...
        printf("RX[%zu]: %d samples, dt=%lli ns\n", blk, sr, timeNs - last_time);
        last_time = timeNs;
...
void trx_samples_buff_repeat(set_sdr *device, const std::vector<complex_d> &samples)
{
    ...
    FILE *file2 = fopen("../resurse/out/txdata_qpsk.pcm", "wb");
    if (!file2) {
        printf("Failed to open txdata.pcm\n");
        free(rx_buff);
        return;
    }
    ...
    int16_t tx_buffer[2*(device->rx_mtu)];

    for (int i = 0; i < device->rx_mtu; i++) {
        if (i < samples.size()) {
    ...
    fwrite(tx_buffer, sizeof(int16_t), 2 * device->rx_mtu, file2);
